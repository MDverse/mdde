...
def config_options_mdp(data_filtered: pd.DataFrame, page_size: int) -> dict: 
...
def user_interaction() -> None:
    ...
    with col_keyup:
        search = st_keyup("MDP files search", placeholder=placeholder)
    columns = st.columns([2 if i == 0 or i == 1 else 1 for i in range(10)])
    with columns[0]:
        show_all = st.checkbox("Show all", key="mdp")
        if show_all:
            search = " "
